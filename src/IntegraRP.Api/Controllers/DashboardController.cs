...
(SELECT count(*) FROM integrarp.tarefa x JOIN t ON t.tenant_id=x.tenant_id WHERE x.excluido_em IS NULL AND x.status='pendente') AS tarefasPendentes,
(SELECT count(*) FROM integrarp.tarefa x JOIN t ON t.tenant_id=x.tenant_id WHERE x.excluido_em IS NULL AND x.status='pendente' AND x.vencimento_em < now()) AS tarefasAtrasadas,
