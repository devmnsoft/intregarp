namespace IntegraRP.Contracts.Auth;
...
public sealed record AuthResponse(string AccessToken, string RefreshToken, DateTimeOffset ExpiresAt, Guid SessionId, AuthUserDto Usuario, AuthTenantDto Tenant, IReadOnlyList<string> Perfis, IReadOnlyList<string> Permissoes);
...
public sealed record ProblemDetailsContract(string? Type, string? Title, int? Status, string? Detail, string? Instance, string? CorrelationId);
