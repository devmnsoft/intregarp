...
public sealed class PostgresV112OperationalRepository(PostgresConnectionFactory connectionFactory, ILogger<PostgresV112OperationalRepository> logger) : IOperationalRuntimeRepository
{
    ...
    public Task<IReadOnlyList<IDictionary<string, object?>>> ListMyTasksAsync(Guid tenantId, Guid userId, Guid? sectorId, CancellationToken ct) => QueryAsync("SELECT id,codigo,titulo,status,vencimento_em AS \"vencimentoEm\",responsavel_email AS \"responsavelEmail\" FROM integrarp.tarefa WHERE tenant_id=@tenant_id AND excluido_em IS NULL AND (responsavel_usuario_id=@user_id OR responsavel_usuario_id IS NULL AND (@sector_id IS NULL OR setor_id=@sector_id)) ORDER BY status, vencimento_em NULLS LAST LIMIT 100;", ct, P("tenant_id", tenantId), P("user_id", userId), P("sector_id", sectorId));
    public Task<IDictionary<string, object?>?> GetTaskAsync(Guid tenantId, Guid id, CancellationToken ct) => FirstAsync("SELECT id,codigo,titulo,descricao,status,vencimento_em AS \"vencimentoEm\",responsavel_email AS \"responsavelEmail\" FROM integrarp.tarefa WHERE tenant_id=@tenant_id AND id=@id AND excluido_em IS NULL;", ct, P("tenant_id", tenantId), P("id", id));
    public Task<IDictionary<string, object?>?> ClaimTaskAsync(Guid tenantId, Guid id, Guid userId, string? email, CancellationToken ct) => FirstAsync("UPDATE integrarp.tarefa SET status='atribuida', responsavel_usuario_id=@user_id, responsavel_email=@email, atualizado_em=now() WHERE tenant_id=@tenant_id AND id=@id AND status IN ('pendente','atribuida') AND excluido_em IS NULL RETURNING id,codigo,titulo,status,responsavel_usuario_id AS \"responsavelUsuarioId\",responsavel_email AS \"responsavelEmail\";", ct, P("tenant_id", tenantId), P("id", id), P("user_id", userId), P("email", email));
    public Task<IDictionary<string, object?>?> CommentTaskAsync(Guid tenantId, Guid id, AddTaskCommentRequest request, Guid userId, CancellationToken ct) => FirstAsync("INSERT INTO integrarp.tarefa_comentario (tenant_id,tarefa_id,author_user_id,texto) SELECT @tenant_id,t.id,@user_id,@text FROM integrarp.tarefa t WHERE t.tenant_id=@tenant_id AND t.id=@id AND t.excluido_em IS NULL RETURNING id,tarefa_id AS \"taskId\",author_user_id AS \"authorUserId\",texto AS text,criado_em AS \"createdAt\";", ct, P("tenant_id", tenantId), P("id", id), P("user_id", userId), P("text", request.Text.Trim()));
    public Task<IDictionary<string, object?>?> StartTaskAsync(Guid tenantId, Guid id, Guid userId, CancellationToken ct) => FirstAsync("UPDATE integrarp.tarefa SET status='em_execucao', iniciado_em=COALESCE(iniciado_em,now()), atualizado_em=now() WHERE tenant_id=@tenant_id AND id=@id AND responsavel_usuario_id=@user_id AND status='atribuida' AND excluido_em IS NULL RETURNING id,codigo,titulo,status,iniciado_em AS \"iniciadoEm\";", ct, P("tenant_id", tenantId), P("id", id), P("user_id", userId));
    public Task<IDictionary<string, object?>?> SaveTaskFormAsync(Guid tenantId, Guid id, string formJson, CancellationToken ct) => FirstAsync("UPDATE integrarp.tarefa SET formulario_resposta_json=@json::jsonb, atualizado_em=now() WHERE tenant_id=@tenant_id AND id=@id AND status <> 'concluida' AND excluido_em IS NULL RETURNING id,codigo,titulo,status,formulario_resposta_json AS \"formulario\";", ct, P("tenant_id", tenantId), P("id", id), P("json", formJson));
    public Task<IDictionary<string, object?>?> SaveTaskChecklistAsync(Guid tenantId, Guid id, string checklistJson, CancellationToken ct) => FirstAsync("UPDATE integrarp.tarefa SET checklist_resposta_json=@json::jsonb, atualizado_em=now() WHERE tenant_id=@tenant_id AND id=@id AND status <> 'concluida' AND excluido_em IS NULL RETURNING id,codigo,titulo,status,checklist_resposta_json AS \"checklist\";", ct, P("tenant_id", tenantId), P("id", id), P("json", checklistJson));
    public Task<IDictionary<string, object?>?> AddTaskEvidenceAsync(Guid tenantId, Guid id, Guid userId, string evidenceJson, CancellationToken ct) => FirstAsync("INSERT INTO integrarp.tarefa_evidencia (tenant_id,tarefa_id,usuario_id,metadata_json) SELECT @tenant_id,t.id,@user_id,@json::jsonb FROM integrarp.tarefa t WHERE t.tenant_id=@tenant_id AND t.id=@id AND t.status <> 'concluida' AND t.excluido_em IS NULL RETURNING id,tarefa_id AS \"taskId\",metadata_json AS \"metadata\",criado_em AS \"createdAt\";", ct, P("tenant_id", tenantId), P("id", id), P("user_id", userId), P("json", evidenceJson));
    public Task<IDictionary<string, object?>?> CompleteTaskAsync(Guid tenantId, Guid id, CancellationToken ct) => FirstAsync("UPDATE integrarp.tarefa SET status='concluida', concluido_em=COALESCE(concluido_em,now()), atualizado_em=now() WHERE tenant_id=@tenant_id AND id=@id AND status='em_execucao' AND excluido_em IS NULL RETURNING id,codigo,titulo,status,concluido_em AS \"concluidoEm\";", ct, P("tenant_id", tenantId), P("id", id));
    public Task<IDictionary<string, object?>?> CancelTaskAsync(Guid tenantId, Guid id, CancellationToken ct) => FirstAsync("UPDATE integrarp.tarefa SET status='cancelada', atualizado_em=now() WHERE tenant_id=@tenant_id AND id=@id AND status <> 'concluida' AND excluido_em IS NULL RETURNING id,codigo,titulo,status;", ct, P("tenant_id", tenantId), P("id", id));
