    ...
    IUnitOfWork
{
    ...
    Task IProcessElementRepository.RemoveAsync(Guid tenantId, Guid versionId, Guid elementId, CancellationToken ct) { elements.RemoveAll(x => x.Id == elementId); return Task.CompletedTask; }
    ...
    Task IProcessTransitionRepository.RemoveAsync(Guid tenantId, Guid versionId, Guid transitionId, CancellationToken ct) { transitions.RemoveAll(x => x.Transition.Id == transitionId); return Task.CompletedTask; }
    ...
    Task<ProcessInstanceDetailResponse?> IProcessInstanceRepository.GetAsync(Guid tenantId, Guid id, CancellationToken ct) { var inst = instances.FirstOrDefault(x => x.Id == id); return Task.FromResult(inst is null ? null : new ProcessInstanceDetailResponse(inst, tasks.Where(x => x.ProcessInstanceId == id).ToArray(), audit)); }
    ...
    Task<WorkflowTaskDetailResponse?> IWorkflowTaskRepository.GetAsync(Guid tenantId, Guid id, CancellationToken ct) { var task = tasks.FirstOrDefault(x => x.Id == id); return Task.FromResult(task is null ? null : new WorkflowTaskDetailResponse(task, [], [])); }
